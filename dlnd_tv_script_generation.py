...
def get_batches(int_text, batch_size, seq_length):
    ...
    batches_word_length = batches * batch_size * seq_length

    np_text = np.array(int_text[: batches_word_length])
    in_text = np_text.reshape(-1, seq_length)
    
    tar_text = np.roll(np_text, -1).reshape(-1, seq_length)
    output = np.zeros(shape=(batches, 2, batch_size, seq_length), dtype=np.int)
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    argmax_probabilities = np.argmax(probabilities)

    return int_to_vocab[argmax_probabilities]
